...
template <uint32_t C, uint32_t ALL_MAP_CHANNELS>
		...
		const bool render_geo)
{
	...
	float accum_rec[C] = {0};
	float accum_all_map[ALL_MAP_CHANNELS] = {0};
	float accum_depth = 0;
	float dL_dpixel[C];
	float dL_dout_all_map[ALL_MAP_CHANNELS];
	...
	float last_all_map[ALL_MAP_CHANNELS] = {0};
	float last_depth = 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (render_geo)
			{
				...
				// float dchannel_dmeany = ray.y / ray_norm_sqr;
				// float dchannel_dmeanz = 1 / ray_norm_sqr;
				// atomicAdd(&(dL_dmeans3D_view[global_id].x), dL_dchannel * dchannel_dcolor * dchannel_dmeanx);
				// atomicAdd(&(dL_dmeans3D_view[global_id].y), dL_dchannel * dchannel_dcolor * dchannel_dmeany);
				// atomicAdd(&(dL_dmeans3D_view[global_id].z), dL_dchannel * dchannel_dcolor * dchannel_dmeanz);
			}
