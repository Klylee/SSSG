	...
	glm::vec4* dL_drot)
{
	...
	dL_dmean.x += dL_dmeans3D_view[idx].x * viewmat[0] + dL_dmeans3D_view[idx].y * viewmat[1] + dL_dmeans3D_view[idx].z * viewmat[2];
	dL_dmean.y += dL_dmeans3D_view[idx].x * viewmat[4] + dL_dmeans3D_view[idx].y * viewmat[5] + dL_dmeans3D_view[idx].z * viewmat[6];
	dL_dmean.z += dL_dmeans3D_view[idx].x * viewmat[8] + dL_dmeans3D_view[idx].y * viewmat[9] + dL_dmeans3D_view[idx].z * viewmat[10];
...
renderCUDA(
	...
	float3* __restrict__ dL_dmeans3D_view,
	...
	const bool render_geo)
{
	...
	const float3 pix_view = { near * (pixf.x - W * 0.5f) / fx, near * (pixf.y - H * 0.5f) / fy, near};
	...
	__shared__ float3 collected_means3D_view[BLOCK_SIZE];
	...
	if (inside) {
		...
		if(render_geo) {
			...
			// const float transparency = all_map_pixels[3 * H * W + pix_id];
			// const float plane_depth = plane_depth_pixels[pix_id];
			// dL_dout_all_map[3] += dL_dout_plane_depths[pix_id] * (-plane_depth / (transparency * transparency)); 
		}
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			collected_means3D_view[block.thread_rank()] = means3D_view[coll_id];
			for (int i = 0; i < C; i++)
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float3 normal_view = { collected_all_maps[0 * BLOCK_SIZE + j],
								   collected_all_maps[1 * BLOCK_SIZE + j],
								   collected_all_maps[2 * BLOCK_SIZE + j]
								 };
			float3 delta = {mean3D_view.x - pix_view.x, mean3D_view.y - pix_view.y, mean3D_view.z - near};
			float depth = (delta.x * normal_view.x + delta.y * normal_view.y) / normal_view.z + delta.z;
			// float3 normal_view = {0,0,0};
