...
from instant_nsr.systemUtils import update_module_step
...
class NeuSModel(BaseModel):
    def setup(self):
        ...
        # we define the inner density is a constant value sigma_t
        self.inner_sigma = nn.Parameter(torch.tensor(1.0))
        self.belta = nn.Parameter(torch.tensor(1.0))
        self.texture_in = models.make(self.config.texture.name, self.config.texture)
    ...
    def update_step(self, epoch, global_step):
        ...
        update_module_step(self.texture_in, epoch, global_step)
    ...
    def get_inner_density(self, points):
        sdf = self.geometry(points, with_grad=False, with_feature=False)
        inner_density = self.inner_sigma * torch.sigmoid(sdf[..., None] / self.belta)
        return inner_density
    
    def get_alpha(self, sdf, normal, dirs, dists):
        '''
        get the density of the inner area of the translucent objects
        '''
        ...
        inner_density = self.inner_sigma * torch.sigmoid(sdf[..., None] / self.belta)
        inner_alpha = 1 - torch.exp(-inner_density * dists.reshape(-1,1)).view(-1)

        return alpha, inner_alpha
    ...
    def ray_upsampe_hier(self, rays_o, rays_d, gs_depth=None, stratified=True, use_depth_guide=False):
        ...
        t_min, t_max, hitted_rays = ray_aabb_intersect(rays_o, rays_d, self.scene_aabb.unsqueeze(0))
        t_min, t_max, hitted_rays = t_min.squeeze(), t_max.squeeze(), hitted_rays.squeeze()
        ...
        if use_depth_guide:
            ...
            intersected_ray_indices = hitted_rays.nonzero(as_tuple=False).view(-1)
        ...
        ray_indices = ray_indices.expand(-1, n_equispaced+n_equispaced_fine).reshape(-1) # [N_rays * N_samples, ]
    ...
    #     _, t_max, hitted_rays = ray_aabb_intersect(rays_o, rays_d, self.scene_aabb.unsqueeze(0))
    #     t_max, hitted_rays = t_max.squeeze(), hitted_rays.squeeze()
    ...
    #     # far_planes = torch.full_like(near_planes, self.far_plane_bg, device=near_planes.device)
    #     # intervals, samples, termination_planes = traverse_grids(
    #     #     rays_o, rays_d, self.occupancy_grid_bg.binaries, self.occupancy_grid_bg.aabbs,
    #     #     near_planes, far_planes, self.render_step_size, self.cone_angle_bg
    #     # )
    #     # t_starts = intervals.vals[intervals.is_left]
    #     # t_ends = intervals.vals[intervals.is_right]
    #     # ray_indices = samples.ray_indices[samples.is_valid]

    #     t_min = torch.where(t_max > 1e9, self.near_plane_bg, t_max)
    #     near_plane = t_min.max().item()
    #     with torch.no_grad():
    #         if self.config.grid_prune:
    #             ray_indices, t_starts, t_ends = self.occupancy_grid_bg.sampling(
    #                 rays_o, rays_d,
    #                 sigma_fn=sigma_fn,
    #                 near_plane=near_plane, far_plane=self.far_plane_bg, t_min=t_min,
    ...
    #         return {
    #             'comp_rgb': torch.zeros([rays_o.shape[0]], device=rays_o.device),
    #             'opacity': torch.zeros([rays_o.shape[0]], device=rays_o.device),
    #             'depth': torch.zeros([rays_o.shape[0]], device=rays_o.device),
    #             'rays_valid': torch.zeros([rays_o.shape[0]], device=rays_o.device),
    #             'num_samples': torch.tensor([0], dtype=torch.int32, device=rays_o.device)
    #         }
    #     ray_indices = ray_indices.long()
    #     t_origins = rays_o[ray_indices]
    #     t_dirs = rays_d[ray_indices]
    ...
    #     weights, transmittance, opacity = render_weight_from_density(t_starts, t_ends, density, ray_indices=ray_indices, n_rays=n_rays)
    ...
    def forward_(self, rays, gs_depth=None, use_depth_guide=False):
        ...
        alpha, inner_alpha = self.get_alpha(sdf.view(-1), normal, t_dirs, dists)
        # RGB field
        rgb = self.texture_in(feature, t_dirs, normal)
        ...
        #inner
        weights_in, _ = render_weight_from_alpha(inner_alpha, ray_indices=ray_indices, n_rays=n_rays)
        rgb_in = accumulate_along_rays(weights_in, ray_indices=ray_indices, values=rgb, n_rays=n_rays)
        opacity_in = accumulate_along_rays(weights_in, ray_indices=ray_indices, n_rays=n_rays)
        ...
        if False:
            ...
            out_bg = {
                'comp_rgb': torch.zeros_like(rgb_in),
    ...
    def regularizations(self, out):
        ...
        losses.update(self.texture_in.regularizations(out))
        return losses
    ...
    def export(self, export_config):
        ...
        if export_config.export_vertex_color:
            ...
            normal = F.normalize(sdf_grad, p=2, dim=-1)
            rgb = self.texture_in(feature, -normal, normal) # set the viewing directions to the normal to get "albedo"
            mesh['v_rgb'] = rgb.cpu()
        return mesh
