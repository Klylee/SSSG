	...
	const bool render_geo)
{
	...
	const float3 pix_view = { near * (pixf.x - cx) / focal_x, near * (pixf.y - cy) / focal_y, near};
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float3 normal_view = { all_map[collected_id[j] * ALL_MAP + 0],
								   all_map[collected_id[j] * ALL_MAP + 1],
								   all_map[collected_id[j] * ALL_MAP + 2]
								 };
			// float depth = (delta.x * normal_view.x + delta.y * normal_view.y + delta.z * normal_view.z) /
			// 				(normal_view.x * view_dir.x + normal_view.y * view_dir.y + normal_view.z * view_dir.z);
			...
			if (render_geo) {
				// ALL_MAP: 0 ~ 2: normal, 3: alpha
				for (int ch = 0; ch < ALL_MAP - 1; ch++)
					All_map[ch] += all_map[collected_id[j] * ALL_MAP + ch] * alpha * T;
				plane_depth += depth * alpha * T;
			}
